...
async function getPostsByUsedId(url, userId) {
  const allPosts = await request.getPosts(url);
...
async function createNewPost(url, body) {
  const response = await request.postNewPost(url, body);
  return response;
}

const result = await createNewPost(baseUrl, body);
console.log(result); // повинен бути респонс у вигляді { userId: 155, title: 'New title', body: 'New body', id: 101 }
